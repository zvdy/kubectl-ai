...
if [[ "$CPU_REQUEST" != "30m" ]]; then
    echo "Container has incorrect CPU request: $CPU_REQUEST, expected: 30m"
    exit 1
fi

# Verify CPU limit
CPU_LIMIT=$(kubectl get pod resource-limits-pod -n limits-test -o jsonpath='{.spec.containers[0].resources.limits.cpu}')
if [[ "$CPU_LIMIT" != "300m" ]]; then
    echo "Container has incorrect CPU limit: $CPU_LIMIT, expected: 300m"
    exit 1
fi
...
if [[ "$MEMORY_REQUEST" != "30Mi" ]]; then
    echo "Container has incorrect memory request: $MEMORY_REQUEST, expected: 30Mi"
    exit 1
fi

# Verify memory limit
MEMORY_LIMIT=$(kubectl get pod resource-limits-pod -n limits-test -o jsonpath='{.spec.containers[0].resources.limits.memory}')
if [[ "$MEMORY_LIMIT" != "30Mi" ]]; then
    echo "Container has incorrect memory limit: $MEMORY_LIMIT, expected: 30Mi"
    exit 1
fi
